#!/usr/bin/env node
// Auto-correct an operative-guide draft that FAILED the quality gates. Reads the latest gate
// findings + the changed IT guide file(s) (+ EN counterpart), asks Claude to apply the MINIMAL
// targeted edits to resolve exactly those findings (preserving voice, structure, frontmatter,
// the contact CTA, and everything unrelated), and writes the corrected files back dash-normalized.
// Run by the `autofix` job in guide-quality-gates.yml; bounded by MAX_AUTOFIX attempts.
//
// Local dry run (no API call, no writes): node automation/guide-fix-gates.mjs --dry-run <it.md>
...
const files = process.argv.slice(2).filter((a) => /^src\/content\/guides\/[^/]+\.md$/.test(a));
if (!files.length) { console.log("No IT guide files passed; nothing to fix."); process.exit(0); }
...
const sysRules = (() => { try { return readFileSync("automation/guide-system-prompt.md", "utf8"); } catch { return ""; } })();
...
for (const itPath of files) {
  ...
  try {
    ...
    if (m) { enPath = `src/content/guides/en/${m[1]}.md`; if (existsSync(enPath)) en = readFileSync(enPath, "utf8"); }
  } catch {}

  const user = [
    "A Soraia operative guide draft FAILED one or more quality gates. Apply the MINIMAL edits needed to resolve EXACTLY the flagged issues. Do not rewrite the guide, do not change the voice or the argument, do not touch anything unrelated to a finding. If a finding is NOT fixable by editing the text (e.g. the topic substantially overlaps an existing guide), leave the files unchanged and say so in `changes`.",
    "",
    "GATE FINDINGS TO RESOLVE:\n" + findings,
    "",
    "CONSTRAINTS you must keep satisfied: meta description 140-160 chars; title <=60 chars (both include the primary keyword); FAQ 3-8 items; >=3 internal links; body 800-1600 words; EN metrics match IT; keep `draft: true` and `lang` unchanged. The body MUST keep (or gain) the contact CTA inviting an honest expert opinion from Soraia: /parliamone in IT, /en/contact in EN. Invent no numbers, Soraia/client figures must trace to llms.txt; every external regulatory/fiscal fact must be attributed to a named source. Use ONLY the ASCII hyphen, never a typographic dash.",
    ...
    "",
    "VOICE + HONESTY RULES (do not violate):\n" + sysRules.slice(0, 2800),
    "",
    `IT FILE (${itPath}):\n${it}`,

...
${llms.slice(0, 6000)}` },
  ...
  { g: "brand-voice", crit: `Judge VOICE ONLY. FAIL on: any banned phrase ("digital transformation", "rivoluziona", "soluzione magica", "game-changer", empty hype); the "paghi solo se funziona" guarantee being SOFTENED where it is mentioned; or prose that reads like generic AI slop rather than an operationally-honest senior practitioner. Do NOT fail on factual sourcing, metrics, internal links/routes, author bylines, archetype, or whether a client is "approved" — other gates own those, and any client named in llms.txt is a valid published citation. The mere ABSENCE of the guarantee is NOT a failure; only its softening where mentioned is.\n\nVOICE RULES (excerpt):\n${sys.slice(0, 3500)}\n\nPublished clients (valid citations — do not flag these):\n${(llms.match(/\[[^\]]+\]/g) || []).join(" ")}` },
  { g: "seo-structure", crit: `FAIL if: title >60 chars or missing the primary keyword; meta description not 140-160 chars; no short TL;DR / key-takeaways block near the top with standalone quotable bullets (label it in the post's OWN language: IT "In breve:"; EN an English label such as "In brief:", "In short:" or "Key takeaways:", do NOT require the literal Italian "In breve:" on the EN version, and a single clearly-labelled summary block satisfies BOTH this gate and geo-citability); FAQ not 3-7 items; fewer than 3 internal links to published pages; body length outside ~450-1500 words; or EN metrics differ from IT.` },

...
const QUESTIONS: Q[] = [
  {
    ...
    help: "PMI secondo l'UE (Allegato I Reg. 651/2014). ULA (Unita Lavorative Annue) = numero medio di dipendenti a tempo pieno nell'anno, inclusi i soci che lavorano. Attenzione: se fai parte di un gruppo, ai tuoi numeri si aggiungono anche quelli delle altre societa. Per intero se una societa ti controlla oltre il 50% (o tu controlli lei); solo in proporzione alla quota se la partecipazione e tra il 25% e il 50%. Cosi una piccola srl dentro un gruppo grande puo risultare esclusa.",
    options: [
  ...
  },
  {
    ...
    help: "Conta per stabilire la dimensione reale. Se un'altra impresa ti controlla oltre il 50% (o tu controlli lei), i suoi numeri si sommano per intero ai tuoi. Se la partecipazione e tra il 25% e il 50%, si somma solo la quota proporzionale. Sommando, una PMI dentro un gruppo grande puo diventare 'grande impresa' e uscire dal bando.",
    options: [
      { label: "No, siamo autonomi", value: "autonomi", effect: "pass" },
      { label: "Si, ma sommando i numeri restiamo sotto le soglie PMI", value: "sotto", effect: "info", action: "Verificare la dimensione aggregata col gruppo (collegate e associate)" },
      { label: "Si, e sommando diventiamo grande impresa", value: "grande", effect: "blocker", action: "Con l'aggregazione del gruppo si supera la soglia PMI" },
      { label: "Non so", value: "nonso", effect: "info", action: "Verificare l'assetto proprietario e la dimensione aggregata" },
    ],
  },
  {
    id: "piemonte",
    question: "Hai sede legale o un'unita operativa attiva in Piemonte, dove faresti il progetto?",
    help: "Serve una sede o una filiale operativa reale e attiva in Piemonte, dove realizzi l'intervento. La sola sede legale altrove non basta se l'attivita e fuori regione.",
    options: [
      { label: "Si", value: "si", effect: "pass" },
      { label: "No", value: "no", effect: "blocker", action: "Serve una sede o unita operativa attiva in Piemonte" },
    ],
  },
  {
    id: "polizza",
    question: "Hai la polizza assicurativa contro le catastrofi naturali (CAT NAT) attiva?",
    help: "CAT NAT = polizza obbligatoria per legge (L. 213/2023) che copre i danni da eventi naturali come alluvioni, terremoti e frane. Il bando la richiede attiva. E' il requisito che blocca piu aziende: se non ce l'hai, va stipulata prima di presentare la domanda, e di solito si fa in tempo.",
    options: [
      { label: "Si, e attiva", value: "si", effect: "pass" },
      { label: "No, non ancora", value: "no", effect: "fixable", action: "Stipulare la polizza catastrofale (CAT NAT) prima della domanda" },
      { label: "Non so cos'e", value: "nonso", effect: "fixable", action: "Verificare e, se manca, stipulare la polizza catastrofale (CAT NAT)" },
    ],
  },
  {
    id: "durc",
    question: "Sei in regola con i contributi (DURC regolare)?",
    help: "DURC = Documento Unico di Regolarita Contributiva: certifica che sei in regola con INPS e INAIL. E' gratuito, lo scarichi online (o lo fa il tuo consulente del lavoro) in pochi minuti e vale 120 giorni. Se sei una societa di persone (snc, sas), devono essere in regola anche i soci, non solo l'impresa.",
    options: [
      { label: "Si", value: "si", effect: "pass" },
      { label: "No / ho irregolarita", value: "no", effect: "fixable", action: "Regolarizzare il DURC prima di concessione ed erogazione" },
  ...
  },
  {
    ...
    help: "Il diritto annuale e la quota annua dovuta alla Camera di Commercio. Una morosita superiore a 100€ negli ultimi 3 anni comporta il rigetto automatico della domanda. Si sana solo pagando gli arretrati PRIMA della presentazione.",
    options: [
      { label: "Si, tutto pagato", value: "si", effect: "pass" },
      { label: "No / ho arretrati sopra 100€", value: "no", effect: "fixable", action: "Pagare gli arretrati del diritto annuale (morosita oltre 100€ = domanda respinta)" },
  ...
  },
  {
    ...
    question: "Hai gia ricevuto un voucher digitalizzazione di Unioncamere Piemonte nel 2023 o 2024?",
    help: "Chi ha gia ottenuto uno di questi specifici voucher camerali nel 2023 o 2024 non puo richiederlo di nuovo. Altri incentivi (crediti d'imposta, altri bandi) non contano per questa domanda.",
    options: [
      { label: "No", value: "no", effect: "pass" },
      { label: "Si", value: "si", effect: "blocker", action: "Hai gia usato il voucher digitalizzazione 2023/2024 di Unioncamere Piemonte" },
      { label: "Non so", value: "nonso", effect: "info", action: "Verificare di non aver gia ricevuto il voucher 2023/2024" },
    ],
  },
  {
    id: "deminimis",
    ...
    help: "De minimis = il tetto di aiuti pubblici minori che un'impresa puo ricevere: 300.000€ in 3 anni (Reg. UE 2831/2023). Con margine residuo il voucher viene solo ridotto al residuo; con plafond pieno l'aiuto non e concedibile. Contano anche crediti d'imposta e garanzie, e il tetto e sull'impresa unica (somma delle collegate). Si verifica gratis sul Registro Nazionale Aiuti, rna.gov.it.",
    options: [
      { label: "Si / credo di si", value: "si", effect: "pass" },
      ...
      { label: "Ho gia usato circa 300.000€ / plafond pieno", value: "pieno", effect: "blocker", action: "Plafond de minimis esaurito: l'aiuto non e concedibile" },
  ...
  },
  {
    ...
    help: "Il bando ammette la gran parte dei settori: industria, servizi, commercio, artigianato. Sono tipicamente esclusi l'agricoltura primaria e la pesca. Il codice ATECO e la sigla che identifica la tua attivita: se hai dubbi, lo verifichiamo noi.",
    options: [
      { label: "Si (non agricoltura/pesca)", value: "si", effect: "pass" },
      { label: "Agricoltura primaria o pesca", value: "no", effect: "blocker", action: "Il tuo settore (ATECO) e tra quelli esclusi" },
      { label: "Non so", value: "nonso", effect: "info", action: "Verificare che il codice ATECO sia ammissibile" },
    ],
  },
  {
    id: "selfi",
    question: "Hai gia compilato il test SELFI4.0 (autovalutazione digitale) negli ultimi 12 mesi?",
    help: "SELFI4.0 e un test online gratuito delle Camere di Commercio che misura quanto e digitale la tua azienda. E' obbligatorio per la domanda (report firmato digitalmente, fatto negli ultimi 12 mesi) ma si compila subito e gratis. E' anche il primo passo del nostro servizio Misurare.",
    options: [
      { label: "Si", value: "si", effect: "pass" },
      { label: "No / non ancora", value: "no", effect: "fixable", action: "Compilare il test SELFI4.0 (gratuito) e firmare il report" },
      { label: "Non so cos'e", value: "nonso", effect: "fixable", action: "Compilare il test SELFI4.0 (gratuito): serve per la domanda" },
    ],
  },
  {
    id: "dichiarazioni",
    question: "Puoi confermare tutti questi punti?",
    help: "Sono cause di esclusione o revoca previste dal bando, in gran parte NON sanabili: 1) l'impresa e i suoi amministratori non hanno condanne penali definitive rilevanti, interdittive antimafia o misure interdittive ex D.Lgs 231/2001; 2) se hai 15 o piu dipendenti, sei in regola con l'assunzione di persone con disabilita (L. 68/99); 3) non hai forniture in essere con Unioncamere Piemonte e non hai delocalizzato (ne prevedi di farlo); 4) non hai cartelle esattoriali sopra 5.000€ non pagate ne rateizzate (potrebbero sospendere l'erogazione). Se anche solo uno non e chiaro, lo verifichiamo insieme.",
    options: [
      { label: "Si, confermo tutti i punti", value: "si", effect: "pass" },
...
export default function BandoQuiz() {
  ...
  if (!started) {
    return (
      <div className="card !p-8 md:!p-10 shadow-lg text-center">
        <p className="text-[11px] font-semibold tracking-[0.15em] uppercase mb-3" style={{ color: brand }}>Questionario di idoneita</p>
  ...
  if (done && verdict) {
    ...
    const captureSub = v.kind === "nonIdoneo"
      ? "Lascia la mail: verifichiamo se c'e comunque margine e, in ogni caso, ci sono altre strade per digitalizzare con l'AI."
      : "Lascia la mail: verifichiamo la tua idoneita documenti alla mano e ti diciamo esattamente come muoverti prima del 22 ottobre.";
    ...
    return (
      <div className="space-y-6">
        <div className="card !p-8 shadow-lg">
          ...
          {v.kind === "idoneo" && v.dim && (
            <p className="mt-4 text-ink-700 leading-relaxed">
              Per la tua dimensione il bando copre il <b>{v.dim.pct}</b> della spesa (fino a 25.000€), con un investimento minimo di circa <b>{v.dim.min}</b>. Il prossimo passo e la verifica ufficiale sui documenti e la domanda prima del click day.
            </p>
          )}
          {v.kind === "idoneoAzioni" && (
          ...
          {v.kind === "nonIdoneo" && (
            <p className="mt-4 text-ink-700 leading-relaxed">Da una o piu risposte sembra mancare un requisito di base:</p>
          )}
        ...
        <LeadForm
          ...
          heading="Verifica la tua idoneita con noi"
  ...
  return (
    <div className="card !p-8 md:!p-10 shadow-lg">
      ...
      <div className="flex items-start gap-2">
        <h3 className="text-xl md:text-2xl font-semibold text-ink-900 tracking-tight leading-snug flex-1">{q.question}</h3>
        <button onClick={() => setHelpOpen((h) => !h)} aria-label="Cosa significa?"
                className="flex-shrink-0 w-7 h-7 rounded-full inline-flex items-center justify-center text-[14px] font-bold transition-colors"
                style={{ background: helpOpen ? brand : "var(--color-violet-50, #F5EEF7)", color: helpOpen ? "#fff" : brand }}>?</button>
      </div>

      {helpOpen && (
        <div className="mt-4 rounded-xl p-4 text-[14px] leading-relaxed" style={{ background: "var(--color-violet-50, #F5EEF7)", color: "var(--color-ink)" }}>
          {q.help}
        </div>
      )}
